...
// SiteNavLinksSection — same render-prop split ProjectCard.jsx/FooterLinkCard.jsx already use.
export default function SiteNavLinkCard({ link, index, dragHandle, onEdit, onDelete, onToggleEnabled }) {
  return (
    <div className={`dash-skill-card${link.enabled === false ? ' disabled' : ''}`}>
      ...
      {link.enabled === false && (
        <span className="dash-skill-card__disabled-note">
          Disabled — hidden from the Nav bar
        </span>
      )}
