...
import { createSiteNavLink, updateSiteNavLink } from '../../store/siteNavLinksSlice.js'
...
// Same Dialog chrome as FooterLinkFormModal — a flat form.
export default function SiteNavLinkFormModal({ open, editingItem, onClose, onSaved }) {
  const dispatch = useDispatch()
  const { mutationStatus, error } = useSelector((s) => s.siteNavLinks)
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await dispatch(
      editingItem ? updateSiteNavLink({ id: editingItem._id, data }) : createSiteNavLink(data)
    )
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="dash-modal-form">
        <h2 className="dash-modal-title">
          {editingItem ? 'Edit Nav Link' : 'New Nav Link'}
        </h2>
        ...
        <div className="dash-field-column">
          ...
          <FormControlLabel
            ...
            label={<span className="dash-switch-label">Enabled — when off, this link is hidden from the Nav bar</span>}
          />
        </div>
